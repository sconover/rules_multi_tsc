...
def _impl(ctx):
    ...
    uses_node_modules = ctx.attr.uses_node_modules
    ...
    inputs = cumulative_js_result.js_and_sourcemap_files + node_dep_files
...
export default {
  ...
  onwarn(warning) {
    ...
    ctx.action(
        command="echo $(pwd);ln -s %s node_modules; %s %s -c %s" % (
            node_modules_path,
        ...
        tools = [
            ...
            rollup_config_file,
            entrypoint_js_file,
        ] + ctx.attr.rollup_plugins.files.to_list() + node_dep_files
    )

    return [DefaultInfo(files=depset([dest_file]))]

rollup_js_bundle2 = rule(
    ...
    attrs = {
      ...
      "uses_node_modules": attr.string_list(),
